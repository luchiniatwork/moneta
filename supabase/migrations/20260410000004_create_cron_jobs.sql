...
-- In that case, run SELECT moneta.archive_stale() manually or via an external cron.
DO $body$
BEGIN
    ...
    PERFORM cron.schedule(
        ...
        '0 3 * * *',
        $$SELECT moneta.archive_stale()$$
    );
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'pg_cron not available: %. Skipping cron job setup.', SQLERRM;
    RAISE NOTICE 'Run SELECT moneta.archive_stale() manually or via external cron.';
END;
$body$;
